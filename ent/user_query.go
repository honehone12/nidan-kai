...
// QueryMfaQrs chains the current query on the "mfa_qrs" edge.
func (_q *UserQuery) QueryMfaQrs() *MfaQrQuery {
	query := (&MfaQrClient{config: _q.config}).Query()
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMfaQrs: _q.withMfaQrs.Clone(),
...
// WithMfaQrs tells the query-builder to eager-load the nodes that are connected to
// the "mfa_qrs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithMfaQrs(opts ...func(*MfaQrQuery)) *UserQuery {
	query := (&MfaQrClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withMfaQrs; query != nil {
		if err := _q.loadMfaQrs(ctx, query, nodes,
			func(n *User) { n.Edges.MfaQrs = []*MfaQr{} },
			func(n *User, e *MfaQr) { n.Edges.MfaQrs = append(n.Edges.MfaQrs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadMfaQrs(ctx context.Context, query *MfaQrQuery, nodes []*User, init func(*User), assign func(*User, *MfaQr)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mfaqr.FieldUserID)
	}
	query.Where(predicate.MfaQr(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MfaQrsColumn), fks...))
	}))
